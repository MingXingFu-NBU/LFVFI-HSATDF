...
psnr_sai = zeros(test_num,ang,ang);   ssim_sai = zeros(test_num,ang,ang);
...
for scene_ind = 1:test_num
    ...
    for an_u = 1:ang
        for an_v = 1:ang
            ...
            psnr_sai(scene_ind,an_u,an_v) = cal_psnr(im2double(com_sai), im2double(gt_sai));  
            ssim_sai(scene_ind,an_u,an_v) = cal_ssim(im2double(com_sai), im2double(gt_sai));  
        end
    end
...
metric_save_path = 'Quantitative_results/psnr_ssim/';
if (~exist(metric_save_path,'dir'))
    mkdir(metric_save_path);
end

save([metric_save_path,'Proposed_psnr_ssim_metrics.mat'], 'psnr_sai','ssim_sai');